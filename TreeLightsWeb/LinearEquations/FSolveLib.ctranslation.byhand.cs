...
namespace TreeLightsWeb.LinearEquations
{
    public static partial class FSolveLibByHand
    {
        const double DBL_EPSILON = 2.2204460492503131e-16; // NOT double.Epsilon as that's something else
        static readonly double DBL_MAX = double.MaxValue;
        static double sqrt(double x) => Math.Sqrt(x);
        static double pow(double x, double y) => Math.Pow(x, y);
        static double fmax(double x, double y) => Math.Max(x, y);
        static double fmin(double x, double y) => Math.Min(x, y);
        static double fabs(double x) => Math.Abs(x);
        static void printf(string s) => Console.Write(s);


        static void dogleg(int n, double[] r, int lr, double[] diag, double[] qtb,
    double delta, double[] x, double[] wa1, double[] wa2)
        ...
        */
        ...
        static void fdjac1(Action<int, double[], double[]> fcn,
    ...
    int ml, int mu, double epsfcn, double[] wa1, double[] wa2)
        ...
        */
        {
            ...
            */
            ...
            info = hybrd(fcn, n, x, fvec, xtol, maxfev, ml, mu, epsfcn, wa, mode,
                factor, nfev, wa.AsSpan(index).ToArray(), n, wa.AsSpan(6 * n).ToArray(), lr,
                wa.AsSpan(n).ToArray(), wa.AsSpan(2 * n).ToArray(), wa.AsSpan(3 * n).ToArray(), wa.AsSpan(4 * n).ToArray(), wa.AsSpan(5 * n).ToArray());
        ...
        static int hybrd(Action<int, double[], double[]> fcn,
            int n, double[] x,
        ...
        */
        ...
        static void qform(int m, int n, double[] q, int ldq)
        ...
        */
        {
            ...
            */
            ...
            for (j = 0; j < n; j++)
            {
                acnorm[j] = enorm(m, a.AsSpan(j * lda).ToArray());
                rdiag[j] = acnorm[j];
                wa[j] = rdiag[j];
                if (pivot)
                {
            ...
            */
            ...
            for (j = 0; j < minmn; j++)
            {
                ...
                */
                ajnorm = enorm(m - j, a.AsSpan(j + j * lda).ToArray());
                    ...
                    */
                    for (k = j + 1; k < n; k++)
                    {
                        ...
                        if (pivot && rdiag[k] != 0.0)
                        {
                            ...
                            {
                                rdiag[k] = enorm(m - 1 - j, a.AsSpan((j + 1) + k * lda).ToArray());
                                wa[k] = rdiag[k];
                            }
                        }
                    }
        ...
        static void r1mpyq(int m, int n, double[] a, int lda, double[] v, double[] w)
        ...
        */
        ...
        static bool r1updt(int m, int n, double[] s, int ls, double[] u, double[] v,
            double[] w)
